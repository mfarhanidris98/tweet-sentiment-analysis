...
public class JsonToFileNeg {

    public static void main(String[] args) {
        JSONParser parser = new JSONParser();
        try {
            Object obj = parser.parse(new FileReader(new ClassPathResource("/strong-negatives.json").getFile()));
            ...
            while (str != "") {
                ...
                if (i > 21092){
                    if (newStr != "" && newStrSize > 5) {
                        File file = new File(new ClassPathResource("/tweets-labeled").getPath() +"/negative/" + i + ".txt");
                        file.createNewFile();
